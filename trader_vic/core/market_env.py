...
from trader_vic.config import ENV_ADAPTATION, CN_BULL_MEAN_MONTHS, VOLUME_SURGE_THRESHOLD, VOLUME_SHRINK_THRESHOLD
...
class MarketEnvClassifier:
    ...
    @staticmethod
    def volume_trend(daily: pd.DataFrame) -> int:
        """量能趋势 — 20日均量 vs 60日均量
        ...
        vol_60 = float(np.mean(vol[-60:]))
        if vol_60 <= 0:
            return 0
        ratio = vol_20 / vol_60
        if ratio > VOLUME_SURGE_THRESHOLD:
            return 1
        elif ratio < VOLUME_SHRINK_THRESHOLD:
            return -1
        return 0

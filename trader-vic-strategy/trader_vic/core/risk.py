...
from trader_vic.config import TIME_STOP_MAX_BARS, CONSECUTIVE_LOSS_PAUSE
...
class StopManager:
    ...
    def update(self, high: float, low: float) -> str:
        ...
        # 多头：止损只上移
        if self.entry_price < self._stop:  # 做多
            new_stop = max(self._stop, self._highest_price * 0.95)
            self._stop = min(new_stop, self.entry_price * 1.5)  # 上限
        ...
        # 空头：止损只下移
            ...
            new_stop = min(self._stop, self._highest_price * 1.05)
...
class TimeStop:
    ...
    @staticmethod
    def check(bars_held: int, max_bars: int = TIME_STOP_MAX_BARS) -> bool:
        """检查时间止损
...
class ConsecutiveLossPause:
    ...
    def __init__(self, threshold: int = CONSECUTIVE_LOSS_PAUSE, pause_bars: int = 20):
